...
class SteeringAngleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if camera_choice == "left":
            ...
            steering_angle = data_entry.iloc[3] + self.steering_correction  # Adjust for left camera
        elif camera_choice == "right":
            img_path = os.path.join(self.img_dir, data_entry.iloc[2].split('/')[-1])
            steering_angle = data_entry.iloc[3] - self.steering_correction  # Adjust for right camera
        else:  # Center
            img_path = os.path.join(self.img_dir, data_entry.iloc[1].split('/')[-1])
            steering_angle = data_entry.iloc[3]  # No adjustment for center
...
class PilotNetDataModule(pl.LightningDataModule):
    def __init__(self, data_dir, csv_file, batch_size=64, split_ratio=0.8):
        super().__init__()
...
if __name__ == "__main__":
    data_dir = '.../Udacity_Dataset/Original_Images'  # Root directory containing "IMG" folder and CSV file
    csv_file = os.path.join(data_dir, "driving_log.csv")  # Path to the CSV file

    # Define the path to store EmissionsTracker output
    output_dir_cnn = '.../Udacity_Dataset/paper_2/No_encoded_cnn/Emissions/Emissions_model'


    data_module = PilotNetDataModule(data_dir=data_dir, csv_file=csv_file, batch_size=64)
    model = PilotNet(learning_rate=1e-4)
    
    logger_name = 'CNN_PilotNet_Udacity'
